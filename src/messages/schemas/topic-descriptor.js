...
const metadata = Joi.object().keys({
  ...
  allowedPublishers: Joi.object.keys({
    enabled: Joi.boolean(),
    peers: Joi.array().items(Joi.binary()).alternatives()
      .when('enabled', {is: true, then: Joi.array().min(1)})
  }).required(),
  requestToPublish: Joi.object.keys({
    enabled: Joi.boolean(),
    peers: Joi.array().items(Joi.binary())
  }).required(),
  eventLinking: Joi.string().values(Object.values(eventLinking)).required()
